...
// by Pinteresting, based on oringal work by MasterFocus, by tic and Rseding91
...
int Gdip_SetBitmapTransColor(unsigned char * Scan, int Width, int Height, int Stride, unsigned char * Trans, int * MCount, int v)
{
    int x1, y1, index;
    MCount[0] = 0;
    for (y1 = 0; y1 < Height; y1++) {
        for (x1 = 0; x1 < Width; x1++) {
            ...
            if ( Scan[index+2] <= Trans[2]+v
			&&	 Scan[index+2] >= Trans[2]-v
			&&   Scan[index+1] <= Trans[1]+v
            &&   Scan[index+1] >= Trans[1]-v
			&&   Scan[index+0] <= Trans[0]+v
            &&   Scan[index+0] >= Trans[0]-v ) {
                Scan[index+3] = 0;
                MCount[0]++;
            }
...
int Gdip_ImageSearchChannel(int * Foundx, int * Foundy, unsigned char * HayStack, unsigned char * Needle, int nw, int nh, int Stride1, int Stride2, int sx1, int sy1, int sx2, int sy2, int v, int sd, int ch)
{
    int y1, y2, x1, x2, idxN, idxH;
    if ( sd == 1 ) { // [default] top->left->right->bottom (vertical preference)
        for (y1 = sy1; y1 < sy2; y1++) {
            for (x1 = sx1; x1 < sx2; x1++) {
                for (y2 = 0; y2 < nh; y2++) {
                    for (x2 = 0; x2 < nw; x2++) {
                        ...
                        idxH = (4*(x1+x2))+((y1+y2)*Stride1); // haystack index offset
                        if ( !( Needle[idxN+3] == 0
                        ||      Needle[idxN+ch] <= HayStack[idxH+ch]+v
                        &&      Needle[idxN+ch] >= HayStack[idxH+ch]-v) )
                            goto NoMatch1;
                    }
                }
...
int Gdip_ImageSearchChannelAlpha(int * Foundx, int * Foundy, unsigned char * HayStack, unsigned char * Needle, int nw, int nh, int Stride1, int Stride2, int sx1, int sy1, int sx2, int sy2, int v, int sd, int ch)
{
    int y1, y2, x1, x2, idxN, idxH;
    if ( sd == 1 ) { // [default] top->left->right->bottom (vertical preference)
        for (y1 = sy1; y1 < sy2; y1++) {
